...
class RandomVariable (list): # list of tuples, representing the evidence` !!!!
    # key: discrete domain; value: float[0..1]
    def __init__(self, main_variable):
        self.main_variable=main_variable
    def add_evidence (self, **evidence): # add_evidence (C=True,T='Negative')
        self.append (evidence) # we add a dictionnary
    def get_prob (self, hypothesis, **evidence): # get_prob (True, T='Positive')
        num=sum(
            row[self.main_variable]==hypothesis and all(row[key]==value for key,value in evidence.items())
            for row in self)
        denom=sum(
            row[self.main_variable]==ANY and all(row[key]==value for key,value in evidence.items())
            for row in self)
        theta=(num+2)/(denom+4)
        uncertainty_at95p=2*math.sqrt(theta*(1-theta)/(denom+4)) # 2-sigma 95% confidence
        return (theta, uncertainty_at95p)
def ppr (prob, uncertainty_at95p):
    print (f"{prob*100:.2f}% \u00B1 {uncertainty_at95p*100:.2f}%")

cancer=RandomVariable("Cancer")
for _ in range (20):
    cancer.add_evidence (Cancer=True, Test='negative')
for _ in range (80):
    cancer.add_evidence (Cancer=True, Test='positive')
for _ in range (700):
    cancer.add_evidence (Cancer=False, Test='positive')
for _ in range (9200):
    cancer.add_evidence (Cancer=False, Test='negative')
ppr (*cancer.get_prob(True, Test='positive')) # probability of cancer if test is positive
raise SystemExit
